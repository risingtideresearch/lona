...
import { emitTapeJvpWgsl, type TapeJvpVariableWgsl } from "./emit-tape-wgsl";

const WORKGROUP_SIZE = 256;
const MAX_SHADER_WORK = 4_000;

export interface GpuCodegenJvpDeviceKernel {
...
export interface GpuCodegenJvpLayout {
...
function shader(tape: CompiledTape, layout: GpuCodegenJvpLayout): string {
  const estimatedWork = tape.opcodes.length * (1 + layout.numDirections);
  if (estimatedWork > MAX_SHADER_WORK) {
    throw new Error(
      `GPU codegen JVP kernel requires ${estimatedWork} node-directions; single-shader limit is ${MAX_SHADER_WORK}`,
    );
  }
  const emitted = emitTapeJvpWgsl(tape, layout.numDirections, (slot) => {
    const binding = layout.variables[slot];
    if (!binding) {
      throw new Error(`GPU codegen JVP has no binding for variable ${slot}`);
    }
    return binding;
  });
...
export function compileGpuCodegenJvpDeviceKernel(
  tape: CompiledTape,
  layout: GpuCodegenJvpLayout,
): GpuCodegenJvpDeviceKernel {
  if (tape.rootIndices.length !== layout.outputWidth) {
    throw new Error(
      `GPU codegen JVP has ${tape.rootIndices.length} outputs; expected ${layout.outputWidth}`,
    );
  }
  if (layout.variables.length !== tape.varSlots.length) {
    throw new Error(
      `GPU codegen JVP expected ${tape.varSlots.length} variable bindings, got ${layout.variables.length}`,
    );
  }
  const device = requireGpuDevice();
  if (device.limits.maxStorageBuffersPerShaderStage < 6) {
    throw new Error(
      `GPU codegen JVP requires 6 storage bindings; device supports ${device.limits.maxStorageBuffersPerShaderStage}`,
    );
  }
  ...
  return {
    ...
    encode(
      ...
      if (!Number.isInteger(count) || count < 0) {
        throw new Error(`invalid GPU codegen JVP count: ${count}`);
      }
      if (count === 0) return;
      ...
      for (const key of Object.keys(expected) as Array<keyof typeof expected>) {
        if (supplied[key] < expected[key]) {
          throw new Error(
            `GPU codegen JVP ${key} has ${supplied[key]} bytes; expected ${expected[key]}`,
          );
        }
        if (expected[key] > device.limits.maxStorageBufferBindingSize) {
          throw new Error(
            `GPU codegen JVP ${key} exceeds the ${device.limits.maxStorageBufferBindingSize} byte storage binding limit`,
          );
        }
      }
      const dispatchLimit =
      ...
      if (outputCount > dispatchLimit) {
        throw new Error(
          `GPU codegen JVP output count ${outputCount} exceeds dispatch limit ${dispatchLimit}`,
        );
      }
